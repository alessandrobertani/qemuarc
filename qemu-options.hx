...
DEF("semihosting", 0, QEMU_OPTION_semihosting,
    ...
    QEMU_ARCH_MIPS | QEMU_ARCH_RISCV | QEMU_ARCH_ARC)
SRST
``-semihosting``
    Enable :ref:`Semihosting` mode (ARM, M68K, Xtensa, MIPS, RISC-V, ARC only).
...
SRST
``-semihosting-config [enable=on|off][,target=native|gdb|auto][,chardev=id][,userspace=on|off][,arg=str[,...]]``
    Enable and configure :ref:`Semihosting` (ARM, M68K, Xtensa, MIPS, RISC-V, ARC
    only).
